...
const Purchasebill = () => {
  ...
  const handleDeleteItem = async (id) => {
    ...
    try {
      await axios.post("purches-delete?", data, {
        params: params,
        headers: {
  ...
  const pdfGenerator = async (id) => {
    ...
    setIsLoading(true);
    try {
      await axios.post("purches-pdf-downloads", data, {
        params: { id },
        headers: {
          Authorization: `Bearer ${token}`,
        },
            ...
            <table className="custom-table">
              ...
              <tbody>
                {filteredList.length === 0 ? (
                  ...
                  .map((row, index) => {
                    return (
                      <tr className='cursor-pointer' hover role="checkbox" tabIndex={-1} key={row.code} >
                        <td>
                          {startIndex + index}
                        </td>
                        {columns.map((column, index) => {
                          ...
                          return (
                            <>
                              <td key={column.id} align={column.align} onClick={() => { history.push(`/purchase/view/${row.id}`); }} style={{ textTransform: "capitalize" }}>
                                {column.format && typeof value === 'number'
                                  ? column.format(value)
                                  : value}
                              </td>
                        ...
                        <td>
                          <div className="flex gap-4 items-center">
                            <VisibilityIcon className='cursor-pointer w-4 h-4' onClick={() => { history.push(`/purchase/view/${row.id}`) }} color="primary" />
