...
const ReturnList = () => {
    ...
    const pdfGenerator = async (id) => {
        ...
        setIsLoading(true);
        try {
            await axios.post("purches-return-pdf", data, {
                params: { id },
                headers: {
    ...
    const AllPDFGenerate = async () => {
        ...
        setIsLoading(true);
        try {
            await axios.post("multiple-purches-return-pdf-downloads", data, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            }).then((response) => {
                                                ...
                                                return (
                                                    <tr hover role="checkbox" tabIndex={-1} key={row.code} >
                                                        ...
                                                        {columns.map((column) => {
                                                            ...
                                                            const dueAmountClass = isDueAmount && row.status === 'Paid'
                                                                ? 'text-black'
                                                                : isDueAmount && value > 0
                                                                    ? 'text-red-500'
                                                                    : 'text-black';
                                                            ...
                                                            return (
                                                                <td key={column.id} align={column.align} onClick={() => { history.push(`/return/view/${row.id}`) }}
                                                                    className="text-lg">
                                                                    <span className={`text ${isStatus ? statusClass : ''} ${isDueAmount ? dueAmountClass : ''}`}>
                                                        ...
                                                        <td>
                                                            {row.cn_amount_bills && row.cn_amount_bills.length > 0 ? (
                                                                <ul  >
                                                                    <Button variant="contained" size='Snall' style={{ background: "rgb(4, 76, 157", fontSize: '12px', textWrap: 'nowrap' }} onClick={() => setOpenAddPopUp(true)}>View CN</Button>
                                                                        ...
                                                                        <div className="flex justify-center items-center h-auto">
                                                                            <div className="bg-white rounded-lg w-full max-w-3xl">
                                                                                <div className="flex justify-between items-center">
                                                                                    ...
                                                                                    >
                                                                                        <CloseIcon onClick={() => { setOpenAddPopUp(false) }} />
                                                                                    </IconButton>
                                                                                </div>
                                                                                <DialogContent>
                                                                                            ...
                                                                                            <div className="bg-white">
                                                                                                <table className="w-full border-collapse custom-table">
                                                                                                    ...
                                                                                                    {row.cn_amount_bills.map((cn_bill, cnIndex) => (
                                                                                                        <tr key={cnIndex}>
                                                                                                            <td className={`text-lg text-black px-35 py-10`} >{cnIndex + 1}</td>
                                                                                                            <td className={`text-lg text-black px-35 py-10`} >{cn_bill.bill_number}</td>
                                                                                                            <td className={`text-lg text-black px-35 py-10`} >{cn_bill.amount}</td>
                                                                                                        </tr>
                                                                                                    ))}
                                                                                                </table>
